...
Step 5: determine the ICC intervals by extracting the components of the network graph list. Use the 'getICC' function defined earlier.
```{r}
...
icc3_i = getICCs(imp_data,ccs_imp,3)
icc4_i = getICCs(imp_data,ccs_imp,4)
icc5_i = getICCs(imp_data,ccs_imp,5)
icc6_i = getICCs(imp_data,ccs_imp,6)
icc_expose_i = getICCs(imp_data, ccs_imp, 4, orderby ="exposure")
```
...
#TODO: I am getting errors when trying to run the serial_mix_est function; I think because there are NAs in the icc_i objects
...
# ggsave(file = "final_figures/FigS1_bootst_SI_sing_imputed.pdf", width = 6, height = 4)
```
