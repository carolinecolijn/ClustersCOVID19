...
**The mean SI (using 4 cases per cluster) is `r myest4[1]`. The standard deviation of the serial intervals is `r myest4[2]`.**
...
**The mean SI using imputed date of symptom onset (estimated with 4 cases per cluster) is `r myest4_i[1]`. The standard deviation of the serial intervals is `r myest4_i[2]`.**
```{r}
### Make a density plot of the ICC estimate
days = seq(from=0, to=10, by=0.1) 
...
**The 95% range for the mean serial interval is (`r myest4_i[1]-1.96*sd(tbestimates_i[,1])`, `r myest4_i[1]+1.96*sd(tbestimates_i[,1])`). This is using imputed date of symptom onset and estimated with 4 cases per cluster.**
