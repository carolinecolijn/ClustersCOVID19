...
cov_net
ggsave("singapore_static_network.pdf",plot=cov_net,device="pdf",path=here("network_diagram"),height=8,width=11,units="in")
```
#####THIS CODE IS A MESS RIGHT NOW-working to fix this right now but I wanted to push it########

Tianjin Data
```{r}
tj=read.csv(here("Tianjin135casesFeb22.csv"))
...
ntj_clean$case_num=str_extract_all(ntj_clean$name,"\\(?[0-9,.]+\\)?")
...
ggsave("tj_static_network.pdf",plot=tj_cov_net,device="pdf",path=here("network_diagram"),height=8,width=11,units="in")
```
