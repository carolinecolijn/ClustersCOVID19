...
return(list(means=mymeans, qs = quantile(mymeans, probs = c(0.025, 0.5, 0.975)), meanmeans=mean(mymeans), sdmeans=sd(mymeans)))
}
```
...
return(data.frame(means=c(out_weib$meanmeans, 
                          out_gamm$meanmeans, 
                          out_lnorm$meanmeans),
           lowerCI=c(out_weib$qs[1], out_gamm$qs[1],
                     out_lnorm$qs[1]),
           upperCI=c(out_weib$qs[3],out_gamm$qs[3],
                     out_lnorm$qs[3])))
}
...
Bootstrap analysis - have left it set to eval=FALSE in the Rmd because it takes time. 
