...
t.dotplot <- tdata %>% 
                  select(c("case_id", "symptom_onset", "end_source", "start_source")) %>% 
...
#tdata <- mutate(tdata, source_group = case_when(!is.na(str_match(Infection_source, "tj|coworker|patient|partner")) ~ "Known relationship", 
#                                                  !is.na(str_match(Infection_source, "wuhan|hubei")) ~ "Wuhan origin",
#                                                  !is.na(str_match(Infection_source, "hebei|dalian")) ~ "Other China travel",
...
tdata <- mutate(tdata, source_group_w = case_when(!is.na(str_match(Infection_source, "wuhan|hubei")) ~ "Wuhan origin",  
                                                  !is.na(str_match(Infection_source, "hebei|dalian")) ~ "Other China travel",
                                                  !is.na(str_match(Infection_source, "train|travel")) ~ "Location unclear travel",
                                                  !is.na(str_match(Infection_source, "tj|coworker|patient|partner")) ~ "Known relationship",
                                                  !is.na(str_match(Infection_source, "mall|store|shopper")) ~ "Mall",
                                                  !is.na(str_match(Infection_source, "unknown|unclear")) ~ "Unknown",
                                                  is.na(Infection_source) ~ "Unknown",
                                                  T ~ "other")) #there should be none of these, so this is just a sanity check!  
table(tdata$source_group_w) 
...
earlydata <- filter(tdata, source_group_w %in% c("Wuhan origin"))
earlyids <- earlydata$case_id
latedata  <- tdata[-which(tdata$case_id == earlyids), ]

#Date range of symptom onset for "Wuhan origin" = "2020-01-14" "2020-02-02"
#Date range of symptom onset for "Location unclear travel" = "2020-01-15" "2020-02-08"
#Date range of symptom onset for "Other China travel" = "2020-01-28" "2020-02-03"
#Date range of symptom onset for "Known relationship" = has 8 NAs; "2020-01-20" "2020-02-19" 
#Date range of symptom onset for "Mall" = has 1 NA; "2020-01-23" "2020-02-09"
#Date range of symptom onset for "Unknown" = has 1 NA; "2020-01-22" "2020-02-02"
