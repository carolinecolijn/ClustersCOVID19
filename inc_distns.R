...
## Plots of the estimated incubation period distributions for the clustering paper

library(ggplot2)
library(viridis)

p1 <- ggplot(data = data.frame(x = c(0, 20)), aes(x)) +
  stat_function(fun = dweibull, n = 1001, size=0.8, args = list(shape=1.88, scale = 7.97), aes(colour = "Singapore, all")) + ylab("") +
  stat_function(fun = dweibull, n = 1001, size=0.8,args = list(shape=2.35, scale = 6.38  ), aes(colour = "Singapore, early")) +
  stat_function(fun = dweibull, n = 1001, size=0.8,args = list(shape=1.93, scale = 8.79 ), aes(colour = "Singapore, late")) +
  scale_y_continuous() +
  stat_function(fun = dweibull, n = 1001, size=0.8,args = list(shape=2.25, scale = 10.15), aes(colour = "Tianjin, all")) + 
  theme_classic() +
  labs(x = "Days")  +
  stat_function(fun = dweibull, n = 1001, size=0.8,args = list(shape=2.73, scale = 7.84 ), aes(colour = "Tianjin, early")) +
  stat_function(fun = dweibull, n = 1001, size=0.8,args = list(shape=3.95, scale = 13.636 ), aes(colour = "Tianjin, late")) +
