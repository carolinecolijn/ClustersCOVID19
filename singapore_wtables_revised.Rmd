...
```{r}
### Stratifying based on "early" = has a date of last exposure vs. "late" = does not have a date of last exposure
# earlydata = spcopy[which(spcopy$has_last ==TRUE),]
# range(earlydata$last_poss_exposure, na.rm = T)  #Dates range from "2020-01-18" to "2020-01-30"

# latedata=spcopy[which(spcopy$has_last==FALSE),]
# range(latedata$last_poss_exposure, na.rm = T) #These cases do not have a date of last possible exposure

#Date range of Singapore dataset
# range(earlydata$last_poss_exposure, na.rm = T) #All before Jan 31st, as with Tianjin dataset
# range(latedata$last_poss_exposure, na.rm = T) #Not applicable, as these cases don't have this info
# range(earlydata$date_onset_symptoms, na.rm = T) 
# range(latedata$date_onset_symptoms, na.rm = T)
    #Hmm...that's interesting that there is overlap between the two groups

### Stratifying based on "early" = has date_onset_symptoms on or before Jan 31, 2020 vs. "late" = has date_onset_symptoms after Jan 31, 2020
# earlydos <- filter(spcopy, date_onset_symptoms <= as.Date("2020-01-31")) #38 cases
# range(earlynew$date_onset_symptoms) #"2020-01-20" "2020-01-31"
# latedos <- filter(spcopy, date_onset_symptoms > as.Date("2020-01-31")) #43 cases
# range(latenew$date_onset_symptoms) #"2020-02-01" "2020-02-16"

#If we want to try the following code with the new split based on date of symptom onset
# earlyprev <- earlydata
# lateprev <- latedata
# earlydata <- earlydos
# latedata <- latedos

###Can do dotplots to explore the distribution of disease progression dates in our dataset
sp.dotplot <- spdata %>% 
                  select(c("CaseID", "date_onset_symptoms", "presumed_infected_date", "last_poss_exposure")) %>% 
                  pivot_longer(cols = c("date_onset_symptoms", "presumed_infected_date", "last_poss_exposure"),
                               names_to = "disease.progression",
                               values_to = "dates") %>% 
                  mutate(CaseID = as.factor(CaseID),
                         disease.progression = as.factor(disease.progression))

dotplot.key <- data.frame(dz.prog = c("date_onset_symptoms", "last_poss_exposure", "presumed_infected_date"),
                          dz.cols = c("darkred", "snow4", "steelblue"),
                          dz.shapes = c(1, 0, 4))

ggplot(sp.dotplot, aes(x = dates, y = CaseID)) +
  geom_point(aes(shape = disease.progression, color = disease.progression), size = 2) +
  ...
  scale_color_manual(values = dotplot.key$dz.cols) +
  scale_shape_manual(values = dotplot.key$dz.shapes)
    #The 75 missing values are from the 'last_poss_exposure' column
```

Hmmm; when do data exploration with dotplot above and visNetwork graphs below, it seems that splitting up the data by date of symptom onset on/before Jan 31 greatly affects the serial interval estimates, likely because it is splitting up the contact path through the largest cluster (Life Church and Grace Assembly groups) between the datasets and thus underestimating possible range of date of symptom onset between infector and infectee. Perhaps a more accurate way to capture differences between "early" and "late" cases in Singapore is through presumed reason for infection: travel from Wuhan (which would have occurred prior to implementation of more strict isolation and distancing procedures) and those from other sources (i.e. known contact with another confirmed COVID-19 case, or association with a group gathering). **Note that the "late" dataset isn't actually later in the timing of the outbreak as the range of date of symptom onset is from Jan 21 to Feb 16, 2020.** However, the epidemiological factors influencing serial interval could differ in Wuhan vs. Singapore, so seems like a more meaningful split.

However, this split is also not without its own possible biases. For instance, we only have the date of the last possible exposure (vs. earliest possible exposure) and the potential for missclassification in the presumed source of infection (i.e. as there maybe other cases that traveled to Wuhan but were presumed to be infected through a gathering at a church or through contact with another known case). It also means that there are far more cases in the "late" dataset that includes all social relationships vs. the "early" dataset that includes only known Wuhan travel.  

```{r}
### Need to group based on presumed source of infection
# Turn 'presumed_reason' into lower case and get trim any whitespace so don't have issues with case sensitivity, etc
spcopy$presumed_reason <- str_to_lower(spcopy$presumed_reason)
spcopy$presumed_reason <- str_trim(spcopy$presumed_reason)
table(spcopy$presumed_reason)
sum(is.na(spcopy$presumed_reason)) #14 NAs
...
spcopy <- mutate(spcopy, presumed_reason_group = case_when(!is.na(str_match(presumed_reason, "symptom onset|via")) ~ "Known relationship",
...
table(spcopy$presumed_reason_group)


### Now split into "early" and "late" datasets 
  #based on presumed reason for infection being Wuhan travel ("early") vs. non-Wuhan or travel related ("late")
earlydata <- filter(spcopy, presumed_reason_group == "Wuhan travel") #17 cases
range(earlydata$date_onset_symptoms) #"2020-01-20" "2020-01-30"

latedata <- filter(spcopy, presumed_reason_group != "Wuhan travel") #64 cases
range(latedata$date_onset_symptoms) #"2020-01-21" "2020-02-16"
```


Fit early and late data to the three distributions:
...
**TODO**
```{r check timeline of related cases dates}
...
```{r}
fedges$arrows <- "to" #try this to put in arrows!
...
#TODO: re-run with N = 100 to match Tianjin script?
...
This section is to determine what impact dividing the dataset based on "early" cases (for Singapore data, "early" is defined as cases where the presumed source of infection is travel from Wuhan) vs "late" cases (for Singapore data, "late" is defined as cases where the presumed source of infection is not from travel or Wuhan).
...
```{r}
nodes_early <- earlydata$CaseID
## How to extract caseIDs from related_cases column - there are multiple values in some cells, separated by commas
earlydata$related_cases #7 max within one cell
# Split into separate columns
earlydata <- separate(earlydata,
                      col = related_cases,
                      into = paste("contactID", 1:7, sep = "_"),
                      fill = "right")
# Turn into numeric values
earlydata <- mutate(earlydata, 
                    contactID_1 = as.numeric(contactID_1),
                    contactID_2 = as.numeric(contactID_2),
                    contactID_3 = as.numeric(contactID_3),
                    contactID_4 = as.numeric(contactID_4),
                    ...
                    contactID_7 = as.numeric(contactID_7))
# Select down to columns of interest
edges_early <- select(earlydata, c(CaseID, starts_with("contactID")))
# Remove rows with NAs for at least one contact
edges_early <- filter(edges_early, !is.na(edges_early$contactID_1)) #13 CasesIDs with 1 or more possible contacts

edges_dfearly = data.frame(from = 2, to = 1) 

for (n in 1:nrow(edges_early)) {  #13
 for (k in 2:ncol(edges_early)) {  #8
   if (!is.na(edges_early[n,k])) {
     edges_dfearly = rbind(edges_dfearly, c(edges_early[[n,k]], edges_early[[n,1]])) 
     }  
   }
}

edges_dfearly = edges_dfearly[-1,]


# create undirected graph by removing duplicates
undir_early = data.frame(from = pmin(edges_dfearly[,1], edges_dfearly[,2]), 
                   to = pmax(edges_dfearly[,1], edges_dfearly[,2]))
undir_early = unique(undir_early)
#undir_early = undir_early[-which(undir_early[,1] == undir_early[,2]), ]  #this evaluates to 0 with early data so omit
e_edges = data.frame(from = paste("case", undir_early[ ,1], sep = ""), 
                     to = paste("case", undir_early[ ,2], sep = ""))
...
```{r}
e_edges$arrows <- "to" #try this to put in arrows!
nodes_dfearly <- data.frame(id = paste("case", earlydata$CaseID, sep=""), 
                            label = earlydata$CaseID, 
                            group = earlydata$presumed_reason_group)
glimpse(nodes_dfearly)
earlydata$graphID = paste("case", earlydata$CaseID, sep="")
visNetwork(nodes_dfearly, e_edges) %>% visLegend() 
```

Early: Estimate the serial interval using the ICC method from Vink et al. 
```{r}
# 1. First construct a graph from edgelist
netgraph_e = graph_from_edgelist(as.matrix(e_edges[ ,1:2]), directed = FALSE)
ccs_e = components(netgraph_e)

earlydata$component = vapply(earlydata$graphID, function(x)
  { if (x %in% names(ccs_e$membership)) { return(ccs_e$membership[match(x, names(ccs_e$membership))])
  } else { 
    return(NA)}}, FUN.VALUE = 3)
...
icc3_e = getICCs(earlydata, ccs_e, 3)
icc4_e = getICCs(earlydata, ccs_e, 4)
icc5_e = getICCs(earlydata, ccs_e, 5)
icc6_e = getICCs(earlydata, ccs_e, 6)
icc_expose_e = getICCs(earlydata, ccs_e, 4, orderby ="exposure")
...
```{r,eval=FALSE}
#4. Plot the density function for the ICC estimate (using 4 cases per cluster)
days = seq(from=0, to=10, by=0.1) 
early.density= dnorm(days, mean = myest4_e[1], sd = myest4_e[2])
ggplot(data=data.frame(days=days, density=early.density), aes(x=days,y=density)) + geom_line() + ggtitle("ICC estimate of the early Singapore cluster serial interval")
#ggsave(file="final_figures/sing_early_serialint.pdf", height = 4, width = 6)
```

The mean SI for the early Singapore dataset is `r myest4_e[1]`. The standard deviation of the serial intervals for the early Singapore dataset is `r myest4_e[2]`. These estimates are constrained by the same restrictions as mentioned with the whole Singapore dataset SI estimates.
...
#This loop still stops A LOT with early data due to NA and NaN errors and needs to be restarted at the following iteration many times
for (kk in 1:Nboot) {
  bdata_e = sample(x = icc4_e, 
                   size = length(icc4_e), 
                   replace = T)
  bestimates_e = rbind(bestimates_e, serial_mix_est(data=bdata_e, N=50, startmu=10, startsig =4))
  print(paste("loop iteration #", kk, sep = ": "))
}

# save(bestimates_e, file = "Testing/sing_boots_100_early_wuhan.Rdata")
...
```{r, eval=TRUE}
load("Testing/sing_boots_100_early_wuhan.Rdata") # in case in Rmd with above evals set to FALSE ...currently this file does NOT exist
hist(bestimates_e[,1],breaks = 30)
bootdf_e = data.frame(mu=bestimates_e[,1], 
                      sig=bestimates_e[,2])
ggplot(bootdf_e, aes(x=mu, y=sig)) + geom_point()
ggplot(bootdf_e, aes(x=mu)) + geom_histogram()
#ggsave(file = "final_figures/bootst_SI_early_wuhan_sing.pdf", width = 6, height = 4)
```
...
```{r}
# Late
nodes_late <- latedata$CaseID
## How to extract caseIDs from related_cases column - there are multiple values in some cells, separated by commas
latedata$related_cases #7 max within one cell
# Split into separate columns
latedata <- separate(latedata,
                      col = related_cases,
                      into = paste("contactID", 1:7, sep = "_"),
                      fill = "right")
# Turn into numeric values
latedata <- mutate(latedata, 
                    contactID_1 = as.numeric(contactID_1),
                    contactID_2 = as.numeric(contactID_2),
                    contactID_3 = as.numeric(contactID_3),
                    contactID_4 = as.numeric(contactID_4),
                    ...
                    contactID_7 = as.numeric(contactID_7))
# Select down to columns of interest
edges_late <- select(latedata, c(CaseID, starts_with("contactID")))
# Remove rows with NAs for at least one contact
edges_late <- filter(edges_late, !is.na(edges_late$contactID_1)) #13 CasesIDs with 1 or more possible contacts

edges_dflate = data.frame(from = 2, to = 1) 

for (n in 1:nrow(edges_late)) {  #30
 for (k in 2:ncol(edges_late)) {  #8
   if (!is.na(edges_late[n,k])) {
     edges_dflate = rbind(edges_dflate, c(edges_late[[n,k]], edges_late[[n,1]])) 
     }  
   }
}

edges_dflate = edges_dflate[-1,]


# create undirected graph by removing duplicates
undir_late = data.frame(from = pmin(edges_dflate[,1], edges_dflate[,2]), 
                   to = pmax(edges_dflate[,1], edges_dflate[,2]))
undir_late = unique(undir_late)
undir_late = undir_late[-which(undir_late[,1] == undir_late[,2]), ]  #this evaluates to index 16 with late data so run here
l_edges = data.frame(from = paste("case", undir_late[ ,1], sep = ""), 
                     to = paste("case", undir_late[ ,2], sep = ""))
```


Late: visNetwork graph with color 'group' based on presumed source of infection.
```{r}
l_edges$arrows <- "to" #try this to put in arrows!
nodes_dflate <- data.frame(id = paste("case", latedata$CaseID, sep=""), 
                            label = latedata$CaseID, 
                            group = latedata$presumed_reason_group)
glimpse(nodes_dflate)
latedata$graphID = paste("case", latedata$CaseID, sep="")
visNetwork(nodes_dflate, l_edges) %>% visLegend() 
```

Late: Estimate the serial interval using the ICC method from Vink et al. 
```{r}
# 1. First construct a graph from edgelist
netgraph_l = graph_from_edgelist(as.matrix(l_edges[ ,1:2]), directed = FALSE)
ccs_l = components(netgraph_l)

latedata$component = vapply(latedata$graphID, function(x)
  { if (x %in% names(ccs_l$membership)) { return(ccs_l$membership[match(x, names(ccs_l$membership))])
  } else { 
    return(NA)}}, FUN.VALUE = 3)

# 2. Use the previously defined getICC function to extract the ICC from this graph object
  #Note that the integers refer to the number of cases per cluster used to determine the ICC
icc3_l = getICCs(latedata, ccs_l, 3)
icc4_l = getICCs(latedata, ccs_l, 4)
icc5_l = getICCs(latedata, ccs_l, 5)
icc6_l = getICCs(latedata, ccs_l, 6)
icc_expose_l = getICCs(latedata, ccs_l, 4, orderby ="exposure")
...
#4. Plot the density function for the ICC estimate (using 4 cases per cluster)
days = seq(from=0, to=10, by=0.1) 
late.density = dnorm(days, mean = myest4_l[1], sd = myest4_l[2])
ggplot(data=data.frame(days=days, density=late.density), aes(x=days,y=density)) + geom_line() + ggtitle("ICC estimate of the late Singapore cluster serial interval")
#ggsave(file="final_figures/sing_late_serialint.pdf", height = 4, width = 6)
```

The mean SI for the late Singapore dataset is `r myest4_l[1]`. The standard deviation of the serial intervals for the late Singapore dataset is `r myest4_l[2]`. These estimates are constrained by the same restrictions as mentioned with the whole Singapore dataset SI estimates.
...
#This loop still stops due to NaN errors and needs to be restarted at the following iteration multiple times
for (kk in 90:Nboot) {     
  bdata_l = sample(x = icc4_l, 
                   size = length(icc4_l), 
                   replace = T)
  bestimates_l = rbind(bestimates_l, serial_mix_est(data=bdata_l, N=50, startmu=10, startsig =4))
  print(paste("loop iteration #", kk, sep = ": "))
}

# save(bestimates_l, file = "Testing/sing_boots_100_LATE_notwuhan.Rdata")
...
```{r, eval=TRUE}
load("Testing/sing_boots_100_LATE_notwuhan.Rdata") # in case in Rmd with above evals set to FALSE ...currently this file does NOT exist
hist(bestimates_l[,1],breaks = 30)
bootdf_l = data.frame(mu=bestimates_l[,1], 
                      sig=bestimates_l[,2])
ggplot(bootdf_l, aes(x=mu, y=sig)) + geom_point()
ggplot(bootdf_l, aes(x=mu)) + geom_histogram()
#ggsave(file = "final_figures/bootst_SI_late_sing.pdf", width = 6, height = 4)
```
