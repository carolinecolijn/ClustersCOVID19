...
#TODO: TRY SOME IMPUTATION METHODS HERE AND SEE HOW IT AFFECTS RESULTS
...
This section is to determine what impact dividing the dataset based on "early" cases (for Singapore data, "early" is defined as cases with a specificied last exposure, which all occurred on or prior to Jan 30th, 2020) vs "late" cases (for Singapore data, "late" is defined as cases without a specified date of last exposure)
```{r}
...
# earlynew <- filter(spcopy, date_onset_symptoms <= as.Date("2020-01-31")) #38 cases
# range(earlynew$date_onset_symptoms) #"2020-01-20" "2020-01-31"
# latenew <- filter(spcopy, date_onset_symptoms > as.Date("2020-01-31")) #43 cases
...
#But as these are different ways of defining early and late, let's just continue with our 
  #prior definition of "early" and "late" for Singapore for now
```

Determine relationships between infectees and putative infectors by defining 'nodes' (= individuals; indicated by the CaseID of that row) and 'edges' (= relationships; indicate by the related_cases column in that lists the cases who are known possible sources of infection for that node) in both the early and late datasets.
...
```{r}
# Turn 'presumed_reason' into lower case and get trim any whitespace so don't have issues with case sensitivity, etc
earlydata$presumed_reason <- str_to_lower(earlydata$presumed_reason)
earlydata$presumed_reason <- str_trim(earlydata$presumed_reason)
table(earlydata$presumed_reason)
sum(is.na(earlydata$presumed_reason)) #0 NAs
# Make a new column where we group the 'presumed_reason' under a label (known relationship, gathering, wuhan travel) for each of the above three groups
earlydata <- mutate(earlydata, presumed_reason_group = case_when(!is.na(str_match(presumed_reason, "symptom onset|via")) ~ "Known relationship",
                                                           !is.na(str_match(presumed_reason, "grace")) ~ "Grace Assembly of God",
                                                           !is.na(str_match(presumed_reason, "grand")) ~ "Grand Hyatt Singapore",
                                                           !is.na(str_match(presumed_reason, "life")) ~ "Life Church",
                                                           !is.na(str_match(presumed_reason, "seletar")) ~ "Seletar Aerospace Heights",
                                                           ...
                                                           is.na(presumed_reason) ~ "Unknown",
                                                           TRUE ~ "other")) #should not be any other, so is just a double check this has run correctly, especially as dataset grows
table(earlydata$presumed_reason_group)
...
**NEED TO COMPLETE REMAINING BOOTSTRAPPING CODE FOR 95% CIs HERE**
...
```{r}
# Turn 'presumed_reason' into lower case and get trim any whitespace so don't have issues with case sensitivity, etc
latedata$presumed_reason <- str_to_lower(latedata$presumed_reason)
latedata$presumed_reason <- str_trim(latedata$presumed_reason)
table(latedata$presumed_reason)
sum(is.na(latedata$presumed_reason)) #14 NAs
# Make a new column where we group the 'presumed_reason' under a label (known relationship, gathering, wuhan travel) for each of the above three groups
latedata <- mutate(latedata, presumed_reason_group = case_when(!is.na(str_match(presumed_reason, "symptom onset|via")) ~ "Known relationship",
                                                           !is.na(str_match(presumed_reason, "grace")) ~ "Grace Assembly of God",
                                                           !is.na(str_match(presumed_reason, "grand")) ~ "Grand Hyatt Singapore",
                                                           !is.na(str_match(presumed_reason, "life")) ~ "Life Church",
                                                           !is.na(str_match(presumed_reason, "seletar")) ~ "Seletar Aerospace Heights",
                                                           ...
                                                           is.na(presumed_reason) ~ "Unknown",
                                                           TRUE ~ "other")) #should not be any other, so is just a double check this has run correctly, especially as dataset grows
table(latedata$presumed_reason_group)
...
**NEED TO COMPLETE REMAINING BOOTSTRAPPING CODE HERE**
